...
export function TodoItem( { todo, toggleTodo, editTodo, deleteTodo }: TodoItemProps ) {
  const [ editing, setEditing ] = state( false );
  const [ input, setInput ] = state<HTMLInputElement | null>( null );
  ...
  function onkeyup( e: Leaner.KeyboardEvent ) {
    if ( e.key == 'Enter' )
      finishEdit();
